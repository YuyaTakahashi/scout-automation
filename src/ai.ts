...
3. **総合評価 (S-D)**:
 ...
 4. **スカウト文作成**: 評価B以上の場合のみ作成。
    - **scoutTitle**: 以下の手順で【キーワード】を作成してください。
      ...
      6. **形式**: 【　】で囲むこと
      7. **絶対NG（出力禁止）**:
         - **禁止ワード**: 「グロース」「牽引」「推進」「改善」「投資」「会計」「広告」「海外事業」「海外」「グローバル」は使用禁止。
         - **禁止キーワード**: 「会計連携」「広告基盤」「マーケティング支援」「海外展開」など、オープンロジの事業領域（物流・SCM）以外の強みをキーワードにしないこと。
         - **ドメイン禁止時の対応**: もし候補者の最大の強みが禁止ドメイン（広告、海外等）にある場合でも、ドメイン名を使わず、その裏にある **「手法や姿勢（UX設計、データ分析、プロジェクト完遂力、要件定義能力等）」** をキーワードにすること。
