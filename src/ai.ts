...
3. **総合評価 (S-D)**:
 ...
 4. **情報の抽出と定義（最優先ステップ）**:
    評価のランク付けを行う前に、まず候補者の経歴から以下の2点を必ず正確に抽出し、言語化してください。
    - **strengths (得意領域)**: 候補者が最も実績を上げている、あるいは深い知識を持つ領域。
    - **aspirations (やりたいこと)**: 候補者が今後挑戦したいこと、あるいはキャリアの方向性。

 5. **スカウトタイトル・メッセージの作成**: 
    前工程で定義した **strengths** と **aspirations** を「唯一のソース」および「思考の出発点」としてください。
    - **【最優先・絶対遵守】禁止ワード(いかなる文脈でも出力禁止)**: 「WEBシステム」「Webシステム」「WEBサービス」「Webサービス」「人材」「求人」「SaaS」「グロース」「牽引」「推進」「改善」「投資」「会計」「広告」「海外事業」「海外」「グローバル」「Overseas」「Global」は、絶対に含めないでください。
    - **キーワード作成手順**:
        1. **分析**: 抽出した **aspirations (志向性/未来)** を最優先で考慮しつつ、**strengths (実績/現在)** を掛け合わせ、「この候補者がオープンロジで何を実現できるか」を定義する。
        2. **キーワード選択**: 定義した内容を基に、以下のカテゴリから最適なものを選択する。
           - 【手法・スタンス】: 「スクラム開発」「アジャイル」「N=1インタビュー」「仮説検証」「SQL/データ分析」「ユーザーディスカバリー」「ユーザーリサーチ」
           - 【ドメイン・難度】: 「B2Bの深掘り」「複雑な業務フロー」「基盤設計」「マルチステークホルダー」「サプライチェーン」「DX」「新規事業」
           - 【フェーズ・役割】: 「0→1」「新規立ち上げ」「基盤刷新」「オーナーシップ」「10→100」
           - 【資質・らしさ】: 「コトに向かう」「解像度の高い言語化」「Outcome重視」
        3. **志向性ごとの変換ルール**:
           - **先端技術・AI**: aspirationsが「先端テクノロジー」「AI」等を含む場合は、キーワードに「AI」「DX」を含める。
           - **社会貢献・社会課題**: aspirationsが「社会貢献」「世の中を良くしたい」等の場合は、**「社会課題」** をキーワードにする。
           - **新規事業**: aspirationsが「新規事業」「0→1」等の場合で、Fintech等の複雑なドメイン経験がある場合は、「複雑なドメイン」「巨大産業」等を掛け合わせる（例：【複雑なドメイン領域の新規事業】）。
        4. **具体的成果の組み合わせ**: 「具体的手段(How) + 成果(Outcome)」の組み合わせでタイトルを完結させる。
           - ✅ 良い例: 【UXでPMFを追求】【データ分析で事業を創る】【スクラムで0→1を創る】【B2Bの課題を定義】【N=1で価値を創る】【社会課題をDXで解決】【社会課題×累計65億調達】【大手×スタートアップ】【複雑なドメイン領域の新規事業】
           - ❌ 悪い例(抽象的/NG): 【事業を創る】【PMFを追求】【Webシステム開発】【WEBサービス改善】【SaaSプロダクト改善】【物流を革新】
        5. **フォールバック規則**: 候補者のプロフィールがいずれのキーワードや「具体的手法」にも明確に合致しない、あるいは訴求が難しい場合は、**デフォルトとして「【社会課題×累計65億調達】」を出力**してください。
        6. **長さ・形式**: 全角4文字〜12文字程度。【　】で囲むこと（フォールバック時を除く）。
        7. **絶対禁止パターン**: 「〇〇で物流を革新」「〇〇で物流を変える」「PM経験募集」「物流を変革」
    - **scoutMessage**: 本文のカスタム部分のみ。宛名は「候補者様」と記し、冒頭挨拶は不要。直接本題から始め、抽出した「得意領域」と「やりたいこと」に具体的に触れ、オープンロジでの活躍イメージを伝えること。
