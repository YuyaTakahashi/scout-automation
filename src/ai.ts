...
3. **総合評価 (S-D)**:
   - **B以上の必須条件**: ミドルまたはジュニアの「条件」を少なくとも1つ満たしており、かつ **「B2B SaaSプロダクトの開発経験」または「ITプロダクトの要件定義・仕様策定の実務経験」** が明確に確認できること。
   - やりたいこととオープンロジのマッチ度 (物流・SCMへの関心)
   - **BtoB SaaS経験の重視**: 
     - 職種名が「PdM」であり「要件定義」の実績があっても、**ドメインが「広告(アドテク)」「マーケティング」「会計」「教育」「ゲーム」等、物流・SCMと関連性が低い場合**は、評価を **必ずC以下** にする。
     - **B以上の必須条件**: 「B2B SaaS（特に複雑な業務フローを伴うもの）」または「物流・製造・SCM関連プロダクト」において、要件定義・仕様策定をリードした実績が明確であること。
   - **未経験ミドルの扱い**: 40代でPdM実務未経験、または受託のPM経験のみの場合は、原則「C」または「D」とする。診断士資格・事業開発実績があっても、上記「重視するドメイン」での仕様策定実績がなければB以上はつけない。
 4. **スカウト文作成**: 評価B以上の場合のみ作成。
    - **scoutTitle**: 以下の手順で【キーワード】を作成してください。
      ...
      6. **形式**: 【　】で囲むこと
      7. **絶対NG（出力禁止）**:
         - **禁止ワード**: 「グロース」「牽引」「推進」「改善」「投資」「会計」「広告」は使用禁止。
         - **禁止キーワード**: 「会計連携」「広告基盤」「マーケティング支援」など、オープンロジの事業領域（物流・SCM）以外の強みをキーワードにしないこと。
