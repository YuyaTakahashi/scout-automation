 ...
 4. **スカウト文作成**: 評価B以上の場合のみ作成。
    - **scoutTitle / titleKeyword**: 
      - **【最優先・絶対遵守】禁止ワード(いかなる文脈でも出力禁止)**: 「SaaS」「グロース」「牽引」「推進」「改善」「投資」「会計」「広告」「海外事業」「海外」「グローバル」「Overseas」「Global」は、scoutTitleおよびtitleKeywordに **絶対に使用しないでください**。
       - **キーワード作成手順**:
         1. **特重視关键词**: 「新規立ち上げ」「0→1」「スクラム開発」「アジャイル」「B2Bの深掘り」など、仕組み作りや手法に関するキーワードを優先してください。「システム開発」「要件定義」などの汎用的な言葉よりも、具体的な進め方や実績を重視します。
         2. **分析**: 候補者の職務経歴から「この人は具体的に何を成し遂げた人か」「何をやりたい人か」を一言で言語化する（強みが禁止ドメインにある場合は、その「手法や姿勢：UX、データ、基盤設計等」に変換する）
         3. **パターン選択**: 「〇〇で事業を変える」「〇〇で社会インフラ革新」「〇〇PdM求む」「〇〇を一任」等の形式にする
         4. **〇〇の作り方**: 候補者の「具体的な実績・手法」から抽出する。
            - ✅ 良い例: 【新規立ち上げを牽引】【スクラムで開発を創る】【B2Bの複雑な課題を紐解く】【0→1のPdM求む】【アジャイルでUXを追求】
            - ❌ 悪い例(汎用的/NG語): 【システム開発を担当】【SaaSプロダクトを改善】【海外展開を牽引】【要件定義を一任】
        5. **長さ・形式**: 全角4文字〜12文字程度。【　】で囲むこと。
        6. **絶対NGパターン**: 「〇〇で物流を革新」「〇〇で物流を変える」「PM経験募集」「物流を変革」
