    ...
    - **scoutTitle / titleKeyword**: 
      - **【最優先・絶対遵守】禁止ワード(いかなる文脈でも出力禁止)**: 「SaaS」「グロース」「牽引」「推進」「改善」「投資」「会計」「広告」「海外事業」「海外」「グローバル」「Overseas」「Global」は、scoutTitleおよびtitleKeywordに **絶対に使用しないでください**。
       - **キーワード作成手順**:
         ...
         2. **抽象的表現の禁止**: **「事業を創る」「PMFを追求」といった具体的な「手段」が不明な表現は「中身がなさすぎる」ため一律禁止**します。必ず「何を使って何をするか」を言語化してください。
         3. **分析**: 候補者の職務経歴から「この人は具体的に何を成し遂げた人か」「何をやりたい人か」を一言で言語化する。
         4. **パターン例**: 「AでBを実現」「AでBを創る」「A PdM求む」「Aを一任」
         5. **〇〇の作り方**: 候補者の具体的な実績から「職種能力（UX, SQL等）」と「目的」を掛け合わせる。
            - ✅ 良い例: 【UXでPMFを追求】【データ分析で顧客を動かす】【スクラムで0→1を創る】【UX設計で事業変革】【B2Bの深部課題を紐解く】
            - ❌ 悪い例(抽象的/NG語): 【事業を創る】【PMFを追求】【システム開発を担当】【SaaSプロダクト改善】【海外展開を牽引】
