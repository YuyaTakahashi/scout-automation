...
3. **総合評価 (S-D)**:
   ...
   - **BtoB SaaS経験の重視**: 
     - 職種名が「PdM」であっても、実態が「受託開発」「事業開発(BizDev)」「営業」「デジタルマーケティング」「グロースハック」「Web広告運用」「広告システム開発管理」「アドテク(AdTech)」のみの場合は、評価を **必ずC以下** にする。
     - 自社開発（特にマルチテナントSaaS）において、エンジニアと協働して「要件定義」「仕様策定」「ロードマップ作成」に深く関わっているかを重視する。
   - **未経験ミドルの扱い**: 40代でPdM実務未経験、または受託のPM経験のみの場合は、原則「C」または「D」とする。診断士資格・事業開発実績があっても、ITプロダクトの「仕様策定」実績がなければB以上はつけない。
