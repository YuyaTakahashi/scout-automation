 ...
 5. **スカウト文作成**: 評価B以上の場合のみ作成。
    - **scoutTitle / titleKeyword**: 
      - **【最優先・絶対遵守】禁止ワード(いかなる文脈でも出力禁止)**: 「WEBシステム」「Webシステム」「WEBサービス」「Webサービス」「人材」「求人」「SaaS」「グロース」「牽引」「推進」「改善」「投資」「会計」「広告」「海外事業」「海外」「グローバル」「Overseas」「Global」は、scoutTitleおよびtitleKeywordに **絶対に使用しないでください**。
       - **キーワード作成手順**:
        1. **分析と優先順位**: 定義した **strengths (現在)** と **aspirations (未来)** を基にするが、特に **「aspirations (やりたいこと)」を最優先的かつ最大限に尊重** してください。
        ...
        3. **抽象的志向への対応（重要）**: 
           - 「世の中を面白く、豊かにしたい」「社会の役に立ちたい」等、**具体的なビジネス手段（BizDev、プロダクト開発、DX等）を伴わない**極めて抽象的な志向のみである場合に限り、キーワードとして「社会課題」を採用してください。
           - 候補者が「価値創造に直接コミットしたい」「現場での事業開発（BizDev）がしたい」など、**具体的または意欲的なビジネス志向を持っている場合は、絶対に「社会課題」へ無理に当てはめないでください**。その場合は志向性に沿った具体的キーワード（例：【新規事業を創る】【事業開発で価値創造】等）を優先してください。
        4. **「具体的手段(How) + 成果(Outcome)」の組み合わせ**: 抽象的な言葉単体（「事業を創る」等）は禁止し、必ず「How」を掛け合わせてください。
        5. **分析**: 候補者の具体的な実績（A）または志向（B）から、オープンロジでの役割（C）へ繋げる一言を作成する。
           - ✅ 良い例: 【UXでPMFを追求】【データ分析で事業を創る】【スクラムで0→1を創る】【B2Bの課題を定義】【N=1で価値を創る】【社会課題をDXで解決】
           - ❌ 悪い例(抽象的/NG): 【事業を創る】【PMFを追求】【Webシステム開発】【WEBサービス改善】【SaaSプロダクト改善】【物流を革新】
        6. **フォールバック規則**: 候補者のプロフィールがいずれのキーワードや「具体的手法」にも明確に合致しない、あるいは訴求が難しい場合は、**デフォルトとして「【社会課題×累計65億調達】」を出力**してください。
        7. **長さ・形式**: 全角4文字〜12文字程度。【　】で囲むこと（フォールバック時を除く）。
        8. **絶対禁止パターン**: 「〇〇で物流を革新」「〇〇で物流を変える」「PM経験募集」「物流を変革」
