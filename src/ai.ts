...
## ジュニア条件
条件1: 年齢が33歳以下 かつ PdM未経験
`;

const EVALUATION_LOGIC = `
## 評価ステップ
1. **Level判定**: 上記条件に基づき Junior/Middle を判定。不明な場合は予測。
2. **スクリーニング**: 
   - 勤務地が「都内・関東近辺」以外 -> 即D評価
   - 年齢が40代後半以上(Middle)/30代半ば以上(Junior) -> 即D評価
3. **総合評価 (S-D)**:
   - やりたいこととオープンロジのマッチ度
   - PdM経験 / BtoB経験 / 現場志向のスタンス
   - **減点対象**: BtoB経験があっても「デジタルマーケティング」や「Web広告運用」のみの経験者は、プロダクト開発（要件定義・仕様策定）の経験不足とみなし、評価を1ランク下げる（例: B -> C）。
4. **スカウト文作成**: 評価B以上の場合のみ作成。
   ...
   - **scoutMessage**: 本文のカスタム部分のみ。「候補者さま」という表現を使用し、冒頭挨拶は不要。直接本題から始め、候補者の経験や実績に具体的に触れ、オープンロジでの活躍イメージを伝える内容にすること
